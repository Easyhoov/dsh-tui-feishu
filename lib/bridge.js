...
export class Bridge {
    ...
    async watchdogTick() {
        ...
        if (state !== 'error' && state !== 'reconnecting' && !silentTooLong)
            return;
        this.options.logger.warn(`watchdog: connection ${state} (ready ${lastReadyAt ?? 'never'}, inbound ${lastInboundAt ?? 'never'}); restarting long connection`);
