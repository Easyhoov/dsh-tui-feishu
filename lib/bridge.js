...
import { buildQuestionCancelledBody, buildQuestionCardBody, buildQuestionPlainText, buildQuestionSettledBody, createQuestionError, QUESTION_ABORTED, summarizeAnswer, } from './user-questions.js';
...
export class Bridge {
     ...
     */
    finalizeOpenThink(state) {
        if (!state.openThink)
            return;
        state.openThink = false;
        if (state.thinkStartedAt === undefined)
            return;
     ...
     */
    askUserQuestion(chatId, request) {
        return new Promise((resolve, reject) => {
            const busy = this.questionBatches.get(chatId);
            if (busy !== undefined && !busy.settled) {
                reject(createQuestionError('another ask_user_question is already pending in this chat; wait for it to be answered first', 'ASK_IN_PROGRESS'));
                return;
            }
            const batch = {
                chatId,
     ...
     */
    async presentQuestionNow(batch) {
        if (batch.settled)
            return;
        const question = batch.questions[batch.index];
        ...
        this.options.logger.info(`user question ${question.id} of chat ${batch.chatId} presented (card ${messageId})`);
    }
    /**
     ...
     */
    async settleQuestionNow(batch, selection) {
        if (batch.settled)
            return;
        const question = batch.questions[batch.index];
        if (question === undefined)
        ...
        if (batch.settled)
            return; // aborted while the ack was in flight
        batch.index += 1;
     ...
     */
    async tryAnswerQuestionText(chatId, text) {
        const batch = this.questionBatches.get(chatId);
        if (batch === undefined || batch.settled)
            return false;
        ...
        const selection = exact !== undefined
     ...
     */
    async handleQuestionAction(action) {
        ...
        if ((sub !== 'choose' && sub !== 'done') || qid === undefined || qid === '')
            return;
        ...
        await this.withQuestionOp(batch, async () => {
            ...
            if (batch.cardMessageId !== expectedMessageId)
                return;
            ...
            const question = batch.questions[batch.index];
            if (question === undefined || question.id !== qid)
                return;
            ...
            if (sub === 'choose' && option !== undefined && option !== '') {
                ...
                if (batch.cardMessageId !== undefined) {
                    try {
                        await this.options.transport.updateCard(batch.cardMessageId, buildQuestionCardBody(question, { toggled: batch.toggled }));
                    }
                    catch (error) {
