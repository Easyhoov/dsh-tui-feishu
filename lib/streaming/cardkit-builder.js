 ...
 */
import { downgradeTables, formatCodeBlock, optimizeMarkdown, prettyJsonOrText, splitLongText, } from '../cardmd.js';
...
function truncateReasoning(text, locale) {
 ...
 */
export function reasoningPanelTitle(rows, locale) {
    const thinkRows = rows.filter((row) => row.kind === 'think');
    if (thinkRows.length === 0)
        return `💭 ${t('thinkingPanel', locale)}`;
    if (thinkRows.some(row => row.durationMs === undefined))
        return `💭 ${t('thinkingPanel', locale)}`;
    const total = thinkRows.reduce((sum, row) => sum + (row.durationMs ?? 0), 0);
    if (total > 0)
        return `💭 ${t('thoughtFor', locale).replace('{}', formatElapsed(total))}`;
