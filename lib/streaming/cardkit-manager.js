...
import { buildCardKitCompleteCard, buildCardKitStreamingCard, buildToolPanel, KIT_ANSWER_ELEMENT, KIT_REASONING_PANEL_ELEMENT, KIT_REASONING_TEXT_ELEMENT, MAX_REASONING_CHARS, reasoningPanelTitle, } from './cardkit-builder.js';
        ...
        try {
            ...
            if (previous === null || snapshot.content !== previous.content) {
                if (!card.streamClosed) {
                    card.seq += 1;
                    try {
                        await this.transport.cardkitStreamElement(card.cardId, KIT_ANSWER_ELEMENT, optimizeMarkdown(downgradeTables(snapshot.content)) || ' ', card.seq);
                    }
                    catch (error) {
                        if (this.isStreamClosedError(error))
                            card.streamClosed = true;
