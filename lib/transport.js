 ...
 */
async function downloadMessageResource(client, messageId, fileKey, resourceType, logger) {
    let response;
    try {
...
export class LarkTransport {
    ...
    async probeImageResourceAccess() {
        try {
            await downloadMessageResource(this.client, 'om_repair_probe', 'img_v3_repair_probe', 'image', this.logger);
    ...
    async uploadAndSendFile(chatId, data, fileName, timeoutMs = 120_000) {
        ...
        await withTimeout(this.createMessage(chatId, 'file', JSON.stringify({ file_key: key })), 15_000, 'file message send');
        return key;
    }
    /**
