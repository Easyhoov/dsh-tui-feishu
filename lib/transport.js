...
export function sniffFileType(data) {
    ...
    if (data.length >= 8 && data[0] === 0xd0 && data[1] === 0xcf && data[2] === 0x11 && data[3] === 0xe0 && data[4] === 0xa1 && data[5] === 0xb1 && data[6] === 0x1a && data[7] === 0xe1)
        return 'ole';
 ...
 */
async function downloadMessageResource(client, messageId, fileKey, resourceType, logger, bounded) {
    let response;
    const attempt = async () => {
        try {
    ...
    const raw = response instanceof ArrayBuffer
...
export class LarkTransport {
     ...
     * self-made `not-found`.
     */
    async getMessage(messageId, timeoutMs = 5_000) {
        const response = (await withTimeout(this.client.im.v1.message.get({
    ...
    async getChat(chatId) {
        return withTimeout(this.client.im.v1.chat.get({ path: { chat_id: chatId } }), 5_000, 'im.v1.chats.get');
    }
    /**
     ...
     */
    async probeImageResourceAccess() {
        try {
            await downloadMessageResource(this.client, 'om_repair_probe', 'img_v3_repair_probe', 'image', this.logger, { timeoutMs: 5_000 });
            // Should not happen: the key is fake, so a success would be surprising.
            return true;
        }
        catch (error) {
            if (error instanceof FeishuApiError) {
                if (error.code === 99991672 || error.code === 234001 || error.code === 91403)
                    return false;
     ...
     */
    async uploadAndSendFile(chatId, data, fileName, timeoutMs = 120_000) {
        if (data.length === 0)
            throw new Error('file is empty');
        if (data.length > 30 * 1024 * 1024)
            throw new Error(`file exceeds the 30 MB platform cap (${data.length} bytes)`);
        const response = (await withTimeout(this.client.request({
            method: 'POST',
            ...
            files: { file: data },
        }), timeoutMs, 'im.v1.file.create'));
        const key = response?.file_key;
        if (key === undefined || key === '')
            throw new Error('file upload returned no file_key');
        const sent = await withTimeout(this.createMessage(chatId, 'file', JSON.stringify({ file_key: key })), 15_000, 'file message send');
