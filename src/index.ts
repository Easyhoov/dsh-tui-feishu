...
import {
  ...
  type QuestionSeat,
  type UserQuestionProviderLike,
  type UserQuestionsServiceLike,
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  let questionSeat: QuestionSeat | undefined
  ...
  const startBridge = (credentials: StoredCredentials): void => {
    ...
    const userQuestions = ctx.get('userQuestions') as UserQuestionsServiceLike | undefined
    if (userQuestions === undefined) {
      logger.warn('[dsh-tui-feishu] userQuestions service unavailable; ask_user_question cannot be answered from Feishu')
    } else {
      const provider: UserQuestionProviderLike = {
        ask: request => {
          ...
          if (chatId !== undefined) return bridge.askUserQuestion(chatId, request)
          const delegate = questionSeat?.delegate
          if (delegate !== undefined) return delegate.ask(request)
          return Promise.reject(
            createQuestionError(
              '这个问题不属于任何飞书会话，且没有可用的本地问卷面板；请在 TUI 或飞书里重新发起',
              'UNANSWERABLE',
            ),
          )
        },
      }
      questionSeat = installUserQuestionsProvider(userQuestions, provider, detail =>
        logger.warn(`[dsh-tui-feishu] ${detail}`),
      )
      logger.info(
        questionSeat.delegate === undefined
