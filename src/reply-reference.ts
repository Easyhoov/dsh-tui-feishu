...
export function unavailableReasonFromError(
  ...
  if (numeric === 401 || numeric === 403) return 'permission-denied'
  if (numeric === 404) return 'not-found'
  if (numeric === 410) return 'deleted'
...
function flattenPost(content: Record<string, unknown>): {
  text: string
  imageCount: number
} {
  let text = ''
...
export function buildReplyReference(
  lookup:
    | { ok: true; message: PlatformMessage }
    | { ok: false; reason?: ReplyUnavailableReason },
): ReplyReference {
  if (!lookup.ok) {
    return {
      note: REPLY_NOTE,
  ...
  let unavailableReason: ReplyUnavailableReason | undefined
  ...
  return {
    ...
    truncated:
      messageId.truncated || authorId.truncated || authorName.truncated || content.truncated,
  }
}

/** Serialize a reference into the escaped `<dsh_im_reply_to>` tag body. */
export function replyTag(reference: ReplyReference): string {
...
export function replyTargetId(message: {
  ...
  if (message.parentId !== undefined && message.parentId !== '') return message.parentId
