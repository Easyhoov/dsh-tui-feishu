...
function collapsiblePanel(options: {
  ...
  if (options.elementId !== undefined) panel['element_id'] = options.elementId
  return panel
}
...
function footerMarkdown(snapshot: CardSnapshot, locale: CardLocale): string {
  const footer: CardFooter | undefined = snapshot.footer
  const parts: string[] = []
  const status =
...
export function buildCardKitCompleteCard(
  snapshot: CardSnapshot,
  locale: CardLocale,
  options: { showReasoning?: boolean } = {},
): Record<string, unknown> {
