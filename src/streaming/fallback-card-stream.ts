...
function isBusinessRejection(error: unknown): error is FeishuApiError {
  return error instanceof FeishuApiError
}

export class FallbackCardStream implements CardStream {
  private engine: CardStream
  private readonly fallbackFactory: () => CardStream
  ...
  async open(chatId: string, title: string): Promise<void> {
    try {
      ...
      if (this.swapped || !this.enabled || !isBusinessRejection(error)) throw error
      this.logger.warn(
        `cardkit turn card rejected at open (code ${error.code}: ${error.message}); ` +
